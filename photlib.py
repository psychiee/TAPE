...
def run_apphot(cx, cy, par):
    """
    -------------------------------
    Purpose
    Do aperture photometry at the coordinate of the image
    -------------------------------
    Inputs
    (cx, cy): x, y coordinate for photometry in the imgae
    img: 2D-array of image data
    par: parameters for photometry (box size, aperture size, sky annulus, and so on)
    -------------------------------
    Outputs
    fstr: text-strings of photometry results
    """
    img = par['IMG']
    ...
    if min(aflx) < 0:
        ...
        prnlog(f'{fidx} {cx:5d} {cy:5d} [FAIL] FWHM={pfwhm:.1f}" PEAK={peak:.1f} A/B={fwhm1 / fwhm2:.1f} {error_msg}')
        return
